document.addEventListener("DOMContentLoaded", () => {
  ...
  const safariSettings = document.getElementById("safariSettings");
  ...
  const loadSettings = () => {
    browser.storage.sync.get(null).then((result) => {
      ttsProviderSelect.value = result.ttsProvider || "safari";
  ...
  const toggleProviderSettings = () => {
    ...
    safariSettings.style.display = provider === "safari" ? "block" : "none";
    geminiSettings.style.display = provider === "gemini" ? "block" : "none";
    commonControls.style.display = provider === "safari" ? "block" : "none";
  };

  const loadSafariVoices = () => {
    if ("speechSynthesis" in window) {
      ...
      browser.storage.sync.get("voice").then((r) => {
  ...
  readBtn.addEventListener("click", async () => {
    try {
      ...
      if (text) {
        ...
        browser.runtime.sendMessage({ action: "speakText", text });
      } else {
  ...
  stopBtn.addEventListener("click", () => {
    browser.runtime.sendMessage({ action: "audioControl", control: "stop" });
    resetPlayer();
  });

  replayBtn.addEventListener("click", () => {
    if (currentText) {
      browser.runtime.sendMessage({ action: "speakText", text: currentText });
    }
  });
